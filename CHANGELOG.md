...
- **`flake.nix` is generated by
  [cl-nix-forge](https://github.com/nerima-lisp/cl-nix-forge)'s
  `mkPackageFlake`.** The org standard was distributed as a template copied by
  hand into every package, and the copies drifted; this repository's copy was
  357 lines re-deriving what the preset now does in one call -- the `:version`
  extraction out of `cl-cli.asd`, `forAllSystems`, the treefmt eval wired to
  both `formatter` and `checks.formatting`, the mkdocs package plus its check,
  the `run-tests.lisp` gate, the `apps.test`/`apps.default` pair and the
  devShell. The hand-rolled `:version` regex went with it: it took the first
  matching line, whereas `fromAsdSystem` requires all three systems in the file
  to agree and refuses to pick a winner if they ever disagree.
  - Test dependencies are now **built** ASDF systems resolved onto
    `CL_SOURCE_REGISTRY` transitively, instead of source trees handed to
    `run-tests.lisp` through `CL_*_SOURCE_DIR`. Declaring that graph surfaced a
    wrong edge the old arrangement had hidden: `cl-boundary-kit` v0.6.0 depends
    on `cl-log-kit`, which never mattered while every `.asd` was registered up
    front and ASDF could resolve them in any order.
  - `nix flake check` gains `overlays.default`, so a downstream `nixpkgs`
    overlay can take `pkgs.cl-cli` directly.
  - The sibling inputs stay `flake = false` and stay pinned to release tags.
    The lock grows 9 -> 15 nodes for `cl-nix-forge` and `paredit-cli`, both of
    which are consumed for their `lib` outputs and so cannot be source trees.
- **`run-tests.lisp` now runs the suite, not just loads it.** It used to leave
  the running to an `--eval '(cl-cli/test:run-tests)'` that every caller had to
  remember to append, so a caller that forgot got a passing run that meant only
  "the tests compiled". The documented invocation is now
  `sbcl --script run-tests.lisp` (or `ecl --shell run-tests.lisp`); the older
  two-step commands still work unchanged, because the script exits during
  `--load` and the trailing `--eval` is never reached.
- **`run-tests.lisp` decides which half of the suite to run by asking ASDF, not
  by looking for sibling directories.** The old test was "did a
  `CL_*_SOURCE_DIR` or `../cl-process-kit/` turn up", which is false under Nix
  even when the systems are present on `CL_SOURCE_REGISTRY` -- so the SBCL gate
  would have silently run the core suite under a name promising the full one.
  It now checks `SB-THREAD` for the capability and `asdf:find-system` for
  reachability, which is true on both paths.
...
- **Tests moved from `tests/` to `t/`,** and the loader moved from
  `tests/run-tests.lisp` to `run-tests.lisp` at the repository root, where
  PACKAGE_STANDARD.md puts the org-wide test entry point.
...
- **`checks.paredit-lint`**, a structural parse gate over every `.lisp` and
  `.asd` file in the tree, from
  [paredit-cli](https://github.com/nerima-lisp/paredit-cli). The suite would not
  catch an unbalanced file: ASDF simply fails to load the system, which reads
  like any other build error and points at the wrong cause. `paredit` is also on
  the `nix develop` shell now, since it is what this repository's Lisp refactors
  go through.
