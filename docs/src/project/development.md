...
flake declares `systems = [ "x86_64-linux" ]` and nothing else — CI runs a
single `ubuntu-latest` job for that reason, and `nix develop`/`nix build`
produce no outputs at all on a non-`x86_64-linux` host. See
